...
public class Pathfinding : MonoBehaviour
{
    ...
    public static void FindPath(Vector2Int start, Vector2Int target, Tilemap tilemap, out List<Node> path)
    {
        ...
        while (openSet.Count > 0)
        {
            ...
            foreach (Node neighbour in GetNeighbours(currentNode, tilemap))
            {
    ...
    private static List<Node> GetNeighbours(Node node, Tilemap tilemap)
    {
        ...
        for (int x = -1; x <= 1; x++)
        {
            for (int y = -1; y <= 1; y++)
            {
                ...
                TileBase current = tilemap.GetTile(new Vector3Int(nx, ny, 0));

                if (current)
                    neighbours.Add(new Node(nx, ny, true));
            }
        }
